...
document.getElementById('payButton').addEventListener('click', function () {
  ...
  fetch('https://konduyt-api.onrender.com/v1/demo/run', {
    ...
    .then(function (data) {
      // The country's OWN catalogue lists every way to pay here, including
      // the methods Konduyt cannot route yet. The ranked options overlay it;
      // reading only those is what made most countries look empty.
    ...
    .catch(function () {
      ...
      document.getElementById('railRows').innerHTML =
        '<tr><td colspan="3">Could not reach the intelligence endpoint. Try again.</td></tr>';
    })
    .finally(function () {
      btn.disabled = false;
      btn.textContent = 'Pay';
...
function renderMethods(methods) {
  ...
  for (var i = 0; i < ordered.length; i++) {
    ...
    // Pay exists only for a method Konduyt can actually charge. A local
    // method it cannot route is still shown -- but never as payable.
    ...
    rows += '<tr class="rail' + (m.executable ? ' rail-executable' : ' rail-unsupported') + (isBest ? ' best' : '') + '" data-key="' + m.key + '">' +
      '<td><span class="rail-name">' + m.label + '</span>' + (isBest ? '<span class="badge">Best value</span>' : '') + '</td>' +
      '<td class="rail-fee">' + fee + feeNote + '</td>' +
      '<td class="rail-action">' + action + '</td></tr>';
  }
  // Only a country with no catalogue at all says this.
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url === "/api/create-payment") {
    // Both fields genuinely arrive here from checkout-page.html's own real
...
export default function DevPanel() {
  ...
  // The country's OWN catalogue is what the popup lists -- every local way to
  // pay, including the ones Konduyt cannot route yet. The ranked options are
  // layered on top as an overlay (see intelligenceMethods.js). Reading only
  // `options` is what made most countries look like they had no payment
  // methods at all.
  ...
  return (
    <div className="panel">
      ...
      {/* Payment intelligence popup — every way this country can pay, with
          the ones Konduyt can actually route marked and priced. */}
      {showIntel && hasMethods && (
        <div className="intel-modal-overlay" onClick={() => setShowIntel(false)}>
          <div className="intel-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="intel-modal-table">
              <div className="intel-modal-row intel-modal-row-head intel-row-3col">
                <span>Pay with</span><span>Transaction fee</span><span></span>
              </div>
              {!hasMethods && (
                <div className="intel-modal-row intel-row-3col">
                  <span className="intel-rail-name">{noMethodsMessage}</span><span></span><span></span>
                </div>
              )}
              {(() => {
                const detectedCarrier = customerPhone.trim() ? detectKenyaCarrier(customerPhone) : null;
                return options.map((o) => {
                  const isBest = bestMethod != null && o.key === bestMethod.key;
                  const requiredCarrier = RAIL_REQUIRED_CARRIER[o.method || o.label] ?? RAIL_REQUIRED_CARRIER[o.label] ?? null;
                  ...
                  const isDisabled = o.executable && requiredCarrier != null && detectedCarrier != null && detectedCarrier !== requiredCarrier;
