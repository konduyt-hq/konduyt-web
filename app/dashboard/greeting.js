// Real time-based greetings for the dashboard, shown fresh on every visit --
// deliberately no cooldown/suppression: every dashboard load is a real
// visit worth greeting, computed from the BROWSER's local clock (never the
// server's, so a developer in Nairobi and one in Lagos get their own real
// time of day at the same UTC moment).

function pick(arr) {
  return arr[Math.floor(Math.random() * arr.length)];
}

const TIME_OF_DAY = {
  earlyBird: [ // 5am - 8am
    (n) => `Hey early bird, ${n}`,
    (n) => `Up with the sun, ${n}?`,
    (n) => `Early start today, ${n}`,
    (n) => `Rise and grind, ${n}`,
  ],
  morning: [ // 8am - 12pm
    (n) => `Morning, ${n}`,
    (n) => `Good morning, ${n}`,
    (n) => `Hey ${n}, let's get into it`,
  ],
  afternoon: [ // 12pm - 5pm
    (n) => `Afternoon, ${n}`,
    (n) => `Hey ${n}`,
    (n) => `Hope your day's going well, ${n}`,
  ],
  evening: [ // 5pm - 9pm
    (n) => `Evening, ${n}`,
    (n) => `Hey ${n}, how's the day been?`,
    (n) => `Winding down, ${n}?`,
  ],
  nightOwl: [ // 9pm - 1am
    (n) => `Up late, ${n}?`,
    (n) => `Burning the midnight oil, ${n}?`,
    (n) => `Night owl mode, ${n}`,
    (n) => `Still going, ${n}? Respect`,
  ],
  veryLate: [ // 1am - 5am
    (n) => `Still going, ${n}? Don't forget to sleep`,
    (n) => `It's really late, ${n} — everything okay?`,
    (n) => `${n}, at this hour even the servers are impressed`,
  ],
};

const GAP_TIER = {
  aFewDays: [ // 3-14 days
    (n) => `Good to see you again, ${n}`,
    (n) => `Welcome back, ${n} — it's been a few days`,
    (n) => `Missed you around here, ${n}`,
  ],
  longTimeNoSee: [ // 14-30 days
    (n) => `Long time no see, ${n}!`,
    (n) => `Look who's back — hey ${n}`,
    (n) => `Well well well, if it isn't ${n}`,
  ],
  reallyLongTime: [ // 30+ days
    (n) => `Well, well, well — welcome back, ${n}!`,
    (n) => `It's been a while, ${n}. Great to have you back`,
    (n) => `${n}?! Is that really you?`,
  ],
  firstEver: [
    (n) => `Welcome to Konduyt, ${n}!`,
    (n) => `Hey ${n}, great to have you here`,
    (n) => `${n}, welcome aboard — let's get you shipping`,
  ],
};

function timeOfDayGreeting(name) {
  const hour = new Date().getHours();
  let tier;
  if (hour >= 5 && hour < 8) tier = TIME_OF_DAY.earlyBird;
  else if (hour >= 8 && hour < 12) tier = TIME_OF_DAY.morning;
  else if (hour >= 12 && hour < 17) tier = TIME_OF_DAY.afternoon;
  else if (hour >= 17 && hour < 21) tier = TIME_OF_DAY.evening;
  else if (hour >= 21 || hour < 1) tier = TIME_OF_DAY.nightOwl;
  else tier = TIME_OF_DAY.veryLate; // 1am - 5am
  return pick(tier)(name);
}

// Always returns a real greeting string -- shown fresh on every dashboard
// visit, never suppressed. A short gap since the last visit still gets a
// real time-of-day greeting rather than nothing at all.
export function buildGreeting(name, previousLastSeenAt, isFirstEver) {
  const displayName = name || 'there';

  if (isFirstEver) {
    return pick(GAP_TIER.firstEver)(displayName);
  }
  if (!previousLastSeenAt) {
    return timeOfDayGreeting(displayName);
  }

  const gapMs = Date.now() - new Date(previousLastSeenAt).getTime();
  const gapDays = gapMs / (1000 * 60 * 60 * 24);

  if (gapDays >= 30) {
    return pick(GAP_TIER.reallyLongTime)(displayName);
  }
  if (gapDays >= 14) {
    return pick(GAP_TIER.longTimeNoSee)(displayName);
  }
  if (gapDays >= 3) {
    return pick(GAP_TIER.aFewDays)(displayName);
  }
  return timeOfDayGreeting(displayName);
}
