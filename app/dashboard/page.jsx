...
export default function DashboardPage() {
  ...
  async function createOrg(e) {
    ...
    setCreating(true)
    try {
      const org = await api.post('/orgs', { name: newName })
    ...
    <div style={{ minHeight:'100vh', background:'var(--bg)', padding:'48px 24px', fontFamily:'Inter,sans-serif' }}>
      <div style={{ maxWidth:'560px', margin:'0 auto' }}>
        ...
        {!showForm
          ...
          : <form onSubmit={createOrg} style={{ display:'flex', gap:'10px', flexWrap:'wrap' }}>
              <input type="text" placeholder="Project name" value={newName} autoFocus onChange={e => setNewName(e.target.value)}
                style={{ flex:1, minWidth:'200px', padding:'10px 14px', background:'rgba(255,255,255,.04)', border:'1px solid var(--border)', borderRadius:'8px', color:'var(--text)', fontSize:'14px', outline:'none' }} />
...
function Loading() {
  return <div style={{ display:'flex', alignItems:'center', justifyContent:'center', minHeight:'100vh', color:'var(--text-muted)', background:'var(--bg)', fontFamily:'Inter,sans-serif' }}>Loading...</div>
}
