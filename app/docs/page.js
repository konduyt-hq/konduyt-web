...
const DOCS = [
  ...
  },
  {
    ...
    body: 'A sourced, versioned model of real 2026 rates for profiled providers: per-method percentage and fixed fees, caps, cross-border surcharges, FX spread, and a settlement bucket (instant, T+1, T+2, T+3). Foreign-currency fixed fees are flagged rather than converted with an invented rate. Unprofiled providers are never shown as free.\n\nA real example worth walking through in detail, because it\'s easy to get subtly wrong: M-Pesa in Kenya. Safaricom publishes two separate, both-real fee tables on the same page -- one for person-to-person transfers, and a completely different one, "Lipa na M-Pesa Buy Goods," for a customer paying a merchant. What Konduyt actually routes is always the merchant case, so it uses the Buy Goods rate: 0.55% of the transaction, capped at a flat KES 200 once the fee would exceed that above roughly KES 36,363, and free entirely under KES 501. An earlier version of this project\'s own demo used the P2P table by mistake -- a real bug, since it meant the demo disagreed with Konduyt\'s own sourced backend data for the identical scenario. Fixed to match. The lesson generalizes: a provider often publishes several genuinely different, correctly-priced tables for different transaction types, and picking the wrong one isn\'t a rounding error, it\'s citing the wrong fact.\n\nOther Kenyan rails follow their own real, independent rates rather than one shared assumption: bank transfers via PesaLink at roughly 0.5%, card payments (and Apple Pay, which rides the same card rails) at roughly 2.9%, PayPal at roughly 3.49%. Ranking these cheapest-first for a real amount is exactly why a flat "card fee" style shortcut would be wrong -- M-Pesa\'s cap means it gets relatively cheaper as the amount grows, while a pure percentage rail like PayPal doesn\'t.\n\nCurrency is resolved the same honest way: Konduyt detects the customer\'s real country and currency, and where that currency isn\'t the rate\'s native one, converts using a real, live FX rate fetched at request time -- never a hardcoded or stale exchange rate standing in for a current one. When live conversion genuinely isn\'t available for a given location, the amount is shown in its native currency instead of silently guessing a rate.',
  },
  {
    id: 'beta-mode',
    group: 'Intelligence',
    title: 'Beta results',
    body: 'The small \u2018Beta\u2019 badge on the payment-intelligence popup means the same thing it means anywhere else you\u2019ve seen it: this specific result is a real, computed estimate, not a live-verified quote for the customer\u2019s own country and provider.\n\nHere is exactly when it appears, and why. Konduyt only ranks a payment method as a real result when it has sourced, verified fee data for that country and provider (see Fee & settlement model). For a country Konduyt hasn\u2019t sourced real data for yet, there is nothing honest to rank \u2014 so instead of showing nothing, or silently reusing a number that was never actually verified for that country, the popup shows Kenya\u2019s own real, connected-provider rates, converted into the customer\u2019s real currency at a live rate, and marks the result Beta. The underlying numbers are always real \u2014 sourced, dated fees for Kenya, genuinely converted \u2014 the badge exists because the country they\u2019re being shown for isn\u2019t the country they were actually measured in.\n\nThis is deliberately the same pattern used across the industry when an answer is a genuine best estimate rather than a directly verified fact: shown plainly, labelled plainly, not hidden and not blocked. A Beta result is never fabricated \u2014 it is always Kenya\u2019s own real, sourced rate, run through a real conversion \u2014 and it always disappears the moment real, sourced data exists for that country instead: the badge is a signal about coverage, not a permanent feature of any given rail.\n\nIf a country you check regularly keeps showing Beta, that\u2019s the real, current state of Konduyt\u2019s own sourced coverage for it, worth reporting \u2014 coverage keeps expanding country by country, the same way Fee & settlement model itself does.',
  },
  {
    id: 'sentinel',
    group: 'Intelligence',
  ...
  },
  {
    id: 'connector-tags',
    group: 'Dashboard',
    title: 'Provider onboarding tags',
    body: 'Every provider card carries a tag saying how much work stands between you and a working set of live keys. The tiers are deliberately coarse, because the underlying fact is qualitative: no provider publishes an "expected onboarding hours" figure, and inventing a precise number would be false confidence. The tag is a link precisely so you can check the reasoning rather than trust a badge.\n\nThe tag answers four things. First, whether onboarding is self-serve or reviewed: can you get keys from a dashboard, or does a human underwriter approve you? Second, the KYC/KYB burden: how much business documentation is demanded, and whether a registered local entity is required. Third, the honest latency range from signup to live keys. Fourth, regional reach: whether the provider is in our catalogue for the country your IP resolves to.\n\nInstant keys are self-serve with no human review; live mode still needs business details, but nothing is queued behind an underwriter. Quick KYC is an online application with standard business documents, usually same-day to a few days. Heavy KYC means underwriting or a contract, often including a local entity and months of processing history, typically weeks. Bank onboarding is not a self-serve product at all: it is a bilateral agreement with the bank, so expect a relationship manager and a bespoke integration. Operator onboarding is the mobile-money case, where you onboard with the operator directly, needing an approved business shortcode and their own compliance review.\n\nA "Not in your country" marker appears only when your own IP-derived country is positively absent from the provider\'s catalogued footprint. That is a statement about our catalogue, not about the provider: a provider may well serve you without us having catalogued it for your country, so the marker is worded to say what we actually know rather than to claim the provider cannot help. Where we have not assessed a provider\'s onboarding at all, it carries no tag rather than a guessed one.',
  },
  {
    id: 'security',
    group: 'Reference',
