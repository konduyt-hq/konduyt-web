      ...
      <nav style={{ flex:1, padding:'8px 0' }}>
        {sections.map((section, si) => (
          <div key={si} style={{ marginBottom:'4px' }}>
            {section.label && (
              <div style={{ fontSize:'10px', fontWeight:700, letterSpacing:'.1em', textTransform:'uppercase', color:'rgba(255,255,255,0.22)', padding:'8px 22px 4px' }}>{section.label}</div>
            )}
            {section.items.map(item => {
              const active = item.exact ? pathname === item.href : (item.href !== '#' && pathname.startsWith(item.href))
              return (
                <Link key={item.label + item.href} href={item.href} style={{ display:'flex', alignItems:'center', gap:'9px', padding:'7px 14px', margin:'1px 8px', borderRadius:'6px', fontSize:'13px', fontWeight: active ? 600 : 400, color: active ? '#EDF0F7' : 'rgba(237,240,247,0.5)', background: active ? 'rgba(255,255,255,0.07)' : 'transparent', textDecoration:'none', transition:'all .12s', borderLeft: active ? '2px solid ' + accent : '2px solid transparent' }}>
